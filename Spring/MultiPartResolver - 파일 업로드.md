...
```
## 문제점과 해결방안
1문제) FileUploadService의 URL 경로는 "image"로 설정하는 것이 좋다. 정적 파일인 assets/images 폴더가 있기 때문에 파일이 충돌이 일어날 수도 있다.   
2문제) mysite-uploads 폴더를 보면 파일이 업로드된 것을 확인할 수 있지만 결과 화면을 보면 액박이 뜬다.   
