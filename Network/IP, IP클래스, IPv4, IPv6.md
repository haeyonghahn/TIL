...
일반 가정집같은 경우에는 PC의 '네트워크 설정'에서 확인하실 수 있는  TCP/IP Protocol을 보면 고정 IP가 아닌 유동 IP로 설정이 되어 있는데, 이는 가정집에서 사용하는 PC의 경우 전원이 Off되어 IP 주소가 필요없는 경우가 대부분이기에 모든 기기에 IP를 부여하게 되면 심각한 IP 부족현상이 나타나기 때문이다. 그래서 DHCP Server를 이용하여 전원이 켜져있는 컴퓨터에게만 IP를 할당하며, 컴퓨터가 Off되면 IP를 회수하게 된다.
