# 정렬과 float
## 정렬
정렬이란 텍스트나 태그를 왼쪽, 가운데 또는 오른쪽에 배치하는 작업이다. 보통 가운데 정렬이나 오른쪽 정렬과 같은 형태를 많이 사용한다.
```css
#parent {
  background: yellow;
  text-align: center;
...
첫 번째 해결방안은 helper를 쓰는 방법이다. 위에서 vertical-align이 옆의 태그를 기준으로 정렬된다고 했으므로 옆에 width가 0이고 높이가 100%인 보이지 않는 태그를 하나 만들어준다. 그럼 다른 태그들이 helper를 기준으로 정렬된다.
